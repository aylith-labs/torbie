<script lang="ts">
 ...
 let { label, value, options, onchange }: { label: string; value: string; options: { value: string; label: string; count?: number }[]; onchange: (value: string) => void } = $props();
 const id = $props.id();
 ...
 async function show(index = Math.max(0, options.findIndex(option => option.value === value))) { active = index; open = true; await tick(); list?.focus(); }
 ...
 function key(event: KeyboardEvent) {
  ...
  if(event.key.length === 1 && !event.ctrlKey && !event.metaKey && !event.altKey && event.key !== ' ') {
   ...
   const index = options.findIndex(option => option.label.toLowerCase().startsWith(typed.toLowerCase()));
...
<div class="select-menu" bind:this={root} onfocusout={event => { if(!root?.contains(event.relatedTarget as Node)) close(); }}>
 ...
 <button bind:this={trigger} type="button" class="select-trigger" aria-labelledby={`${id}-label ${id}-value`} aria-haspopup="listbox" aria-expanded={open} aria-controls={`${id}-list`} onclick={() => open ? close() : show()} onkeydown={key}>
 ...
 {#if open}
  <ul bind:this={list} id={`${id}-list`} class="select-options" role="listbox" tabindex="0" aria-labelledby={`${id}-label`} aria-activedescendant={`${id}-option-${active}`} onkeydown={key}>
   {#each options as option, index}
    <li id={`${id}-option-${index}`} role="option" aria-selected={option.value === value} class:active={active === index} onpointermove={() => active = index} onclick={() => choose(index)} onkeydown={event => { event.stopPropagation(); key(event); }}>
