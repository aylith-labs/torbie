<script lang="ts">
 ...
 export async function openFullScreen(){
  ...
  activate();view='fullscreen';activity=false;splitOpen=false;
  try{if(document.fullscreenElement!==host)await host.requestFullscreen()}catch{/* Fill the viewport when the browser does not support fullscreen. */}
 ...
 onMount(()=>{
  ...
  const fullscreen=()=>{if(view==='fullscreen'&&document.fullscreenElement!==host)void exitView()};
  bindFrameKeys();
  ...
  return()=>{menuObserver.disconnect();if(frame){frame.inert=false;if(frame.contentDocument)frame.contentDocument.documentElement.inert=false;}detachFrameKeys();document.removeEventListener('fullscreenchange',fullscreen);window.removeEventListener('keydown',handleEscape);clearTimeout(timer);observer.disconnect();mutation.disconnect();preference.removeEventListener('change',sync);window.removeEventListener('message',receive)};
 });
</script>
<svelte:window onpointerdown={event=>{if(splitOpen&&!splitControl?.contains(event.target as Node))splitOpen=false}} />
<section class="live-preview" class:compact class:expanded class:immersive={view==='immersive'} data-view={view} bind:this={host} aria-label="Interactive Torbie preview">
 <div class="preview-heading"><div><span class="eyebrow">Try the real interface</span><h2>{compact?'Explore this workspace.':'Your agents. Your terminal.'}</h2></div><div class="preview-tools"><span class="demo-label"><i></i>Real Torbie UI · mock sessions</span><button class="view-toggle fullscreen-toggle" aria-pressed={view==='fullscreen'} onclick={()=>view==='fullscreen'?exitView():openFullScreen()}>{view==='fullscreen'?'Exit full screen':'Full screen'} <span aria-hidden="true">↗</span></button><button class="view-toggle" onclick={openImmersive}>Immersive view <span aria-hidden="true">↗</span></button></div></div>
 ...
 {#if view==='immersive'}<button class="view-toggle exit-immersive" aria-keyshortcuts="Shift+Escape" title="Exit immersive view (Shift+Escape)" onclick={exitView}>Exit immersive view <span aria-hidden="true">×</span></button>{/if}
 <div class="preview-window">
  <iframe bind:this={frame} onload={bindFrameKeys} src={`${base}/preview/${active?'index':'loading'}.html${resetId?`?reset=${resetId}`:''}`} title="Torbie — interactive Angular terminal demo" style="color-scheme:inherit" sandbox="allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox" allow="fullscreen"></iframe>
...
<style>
 ...
 @media(max-width:650px){
  ...
  .live-preview.expanded{padding:10px}.expanded .preview-heading h2,.expanded .demo-label,.expanded .preview-footnote>span,.expanded .preview-actions>span{display:none}.expanded .preview-tools{margin-top:0}.expanded .activity{margin-top:0}.expanded .preview-actions{padding:8px}.expanded .preview-window{height:auto}
 }
 ...
 .exit-immersive span{margin-left:8px}
</style>
