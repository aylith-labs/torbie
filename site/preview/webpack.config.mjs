...
export default {
 ...
 {test:/\.m?js$/,resolve:{fullySpecified:false},use:{loader:require.resolve('babel-loader'),options:{sourceType:'unambiguous',plugins:[createEs2015LinkerPlugin({linkerJitMode:true,fileSystem:{resolve:path.resolve,exists:fs.existsSync,dirname:path.dirname,relative:path.relative,readFile:fs.readFileSync}})],compact:false,cacheDirectory:true}}}
 ]},plugins:[new webpack.NormalModuleReplacementPlugin(/\/lib\/diagnostics$/,path.join(here,'diagnostics.ts')),new webpack.NormalModuleReplacementPlugin(/(?:claudeSessions|claudeActions|herdr|stith|transcriptMetrics)\.service$/,path.join(here,'claude-services.ts')),new webpack.ProvidePlugin({Buffer:['buffer','Buffer'],process:require.resolve('./mock-process.cjs')}),new webpack.DefinePlugin({'process.env.TABBY_BUILD_VERSION':JSON.stringify('1.0.0-demo'),'process.env.TABBY_BUILD_SHA':JSON.stringify('browser demo'),'process.env.TABBY_BUILD_BRANCH':JSON.stringify('demo'),'process.env.TABBY_BUILD_DATE':JSON.stringify(new Date().toISOString()),'process.env.TABBY_BUILD_TIMESTAMP':JSON.stringify(String(Date.now()))})],stats:'errors-warnings'
};
