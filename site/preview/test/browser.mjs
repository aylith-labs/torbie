...
for(const width of [1440,390]){
 ...
 await page.locator('.live-preview').scrollIntoViewIfNeeded();const enable=page.getByRole('menuitem',{name:'Enable Claude Code',exact:true});await expect(page.getByRole('button',{name:'Plugins',exact:true})).toBeEnabled({timeout:30000});const readyMs=Date.now()-start;
 ...
 await page.getByRole('button',{name:'Actions',exact:true}).click();await enable.click();await expect(frame.locator('body')).toContainText('Build the workspace');await page.getByRole('button',{name:'Actions',exact:true}).click();await expect(page.getByRole('menuitem',{name:'Disable Claude Code',exact:true})).toBeVisible();
 await page.screenshot({path:`.review/angular-integrated-${width}-light.png`});
 await page.getByRole('menuitem',{name:'Disable Claude Code',exact:true}).click();
 await page.getByRole('button',{name:'Split pane',exact:false}).click();await expect(frame.locator('demo-terminal')).toHaveCount(9);
 await page.getByRole('button',{name:'Actions',exact:true}).click();await page.getByRole('menuitem',{name:'Run Playwright checks',exact:true}).click();await expect(frame.locator('body')).toContainText('18 passed');
 await page.getByRole('button',{name:'Plugins',exact:false}).click();const toggle=frame.getByRole('switch',{name:'Enable Claude Code',exact:true});await expect(toggle).toBeVisible();await expect.poll(()=>toggle.evaluate(e=>{const r=e.getBoundingClientRect();return r.left>=0&&r.right<=innerWidth})).toBe(true);await toggle.click();await page.getByRole('button',{name:'Actions',exact:true}).click();await expect(page.getByRole('menuitem',{name:'Disable Claude Code',exact:true})).toBeVisible();
