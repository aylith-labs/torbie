...
let phase='startup';const errors=[];page.on('pageerror',error=>{errors.push(error.message);console.error(phase,error.stack)});
...
try {
 ...
 await expect(page.locator('.live-preview')).toHaveAttribute('data-view','fullscreen');await ready();
 ...
 for(const value of ['Right','Bottom','Top']){
  await page.locator('.tab-location button').click();
  await frame.locator('body').evaluate(()=>[...document.querySelectorAll('textarea')].find(element=>element.getClientRects().length)?.focus());
  await expect(page.getByRole('listbox')).toBeFocused();await page.getByRole('option',{name:value,exact:true}).click();
 ...
 phase='reset';await page.getByRole('button',{name:'Reset demo'}).click();await ready();await expect(frame.locator('demo-terminal')).toHaveCount(1);await expect(page.locator('.tab-location button')).toContainText('Left');
 ...
 phase='close all tabs';await frame.locator('body').evaluate(async()=>{for(const tab of [...window.demo.app.tabs])await window.demo.app.closeTab(tab)});
 ...
 phase='guide';await page.goto(url);await page.getByRole('button',{name:'Open Torbie guide',exact:true}).click();await expect(page.getByRole('button',{name:'Park here',exact:true})).toBeVisible();await page.getByRole('button',{name:'Close guide',exact:true}).click();
 ...
 expect(errors).toEqual([]);console.log('Full-screen entry, MCP popover, splits, tab positions, recovery, reset, disclosures, themes and guide: passed.');
