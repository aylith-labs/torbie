...
export class BaseTerminalTabComponent<P extends BaseTerminalProfile> extends BaseTabComponent implements OnInit, OnDestroy {
    ...
    sendInput (data: string|Buffer): void {
        // Narrowed on `typeof`, not `instanceof Buffer`. `Buffer` is generic in
        // current @types/node (`Buffer<ArrayBufferLike>`), and narrowing a
        // `string|Buffer` union by it leaves the union intact — so `Buffer.from`
        // is handed `string|Buffer` and matches none of its four overloads.
        // `typeof data === 'string'` has no such ambiguity.
    ...
    protected async writeRaw (data: string): Promise<void> {
        ...
        if (this.config.store.terminal.detectProgress) {
            ...
            if (!this.alternateScreenActive && percentageMatch) {
                const percentage = percentageMatch[3] ? parseFloat(percentageMatch[2]) : parseInt(percentageMatch[2], 10)
